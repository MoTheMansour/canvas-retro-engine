...
const DEFAULT_SETTINGS: CanvasNesPluginSettings = {
    ...
    masterState: {
        ...
        crtScreenShape: "modern" // 'modern' | 'vintage_bubble'
    },
    sfxConfigs: {}
};
        ...
        if (!this.controllerPadEl || this.currentControllerSystem !== activeSys) {
            ...
            if (activeSys === 'nes') {
                ...
                try {
                    ...
                    const fullImgPath = path.isAbsolute(manifestDir)
                        ? path.join(manifestDir, 'assets', 'psx', 'ps1 controller.png')
                        : path.join(basePath, manifestDir, 'assets', 'psx', 'ps1 controller.png');
